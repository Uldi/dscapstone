...
#Assumption: k = 0!
getNextWordPredictionTable <- function(ngramKBOTables, text) {
    ...
    if (numWords >= 3) {
        last3Words <- paste(words[(numWords-2): numWords], collapse = " ")
        flog.trace("4-gram: %s", last3Words)
        rows4 <- lookupNGram(ngramKBOTables[[4]], last3Words)
    ...
    if (numWords >= 2) {
        last2Words <- paste(words[(numWords-1): numWords], collapse = " ")
        flog.trace("3-gram: %s", last2Words)
        rows3 <- lookupNGram(ngramKBOTables[[3]], last2Words)
    ...
    if (numWords >= 1) {
        lastWord <- words[numWords]
        flog.trace("2-gram: %s", lastWord)
        rows2 <- lookupNGram(ngramKBOTables[[2]], lastWord)
